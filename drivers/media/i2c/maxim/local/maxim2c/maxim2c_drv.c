 ...
 *     1. local deserializer support: max9296/max96716/max96718
	...
	for (loop = 0; loop < 5; loop++) {
		...
		if (ret == 0) {
			if (chipid == maxim2c->chipid) {
				if (chipid == MAX9296_CHIP_ID) {
					dev_info(dev, "MAX9296 is Detected\n");
					return 0;
				}
			...
			const struct i2c_device_id *id)
{
	...
	if (chip_id == MAX9296_CHIP_ID) {
		dev_info(dev, "maxim2c driver for max9296\n");
	} else if (chip_id == MAX96716_CHIP_ID) {
...
static const struct of_device_id maxim2c_of_match[] = {
	{
		.compatible = "maxim2c,max9296",
		.data = (const void *)MAX9296_CHIP_ID
	}, {
