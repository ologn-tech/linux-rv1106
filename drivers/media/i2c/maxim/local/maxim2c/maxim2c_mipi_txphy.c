...
int maxim2c_mipi_txphy_hw_init(maxim2c_t *maxim2c)
{
	...
	/*
	 * Clock master / DPLL programming:
	 *   2x4: Port A clock on PHY B, Port B clock on PHY C
	 *   2x2: Port A clock on PHY A, Port B clock on PHY B
	 */
	if (mipi_txphy->phy_mode == MAXIM2C_TXPHY_MODE_2X2LANES) {
		phy_cfg = &mipi_txphy->phy_cfg[MAXIM2C_TXPHY_ID_A];
		if (phy_cfg->phy_enable)
	...
	if (maxim2c->chipid != MAX9296_CHIP_ID)
