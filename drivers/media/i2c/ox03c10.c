...
#define PIXEL_RATE_WITH_648M		(MIPI_FREQ_648M / OX03C10_BITS_PER_SAMPLE * 2 * OX03C10_LANES)
#define PIXEL_RATE_WITH_720M		(MIPI_FREQ_720M / OX03C10_BITS_PER_SAMPLE * 2 * OX03C10_LANES)
...
#define OX03C10_REG_DGAIN_MID_H		0x354A
#define OX03C10_REG_DGAIN_VS_H		0x358A
...
#define OX03C10_GROUP_UPDATE_END_LAUNCH	0xA0

#define OX03C10_SOFTWARE_RESET_REG	0x0103

#define OX03C10_FETCH_MSB_BYTE_EXP(VAL)	(((VAL) >> 8) & 0xFF)	/* 8 Bits */
#define OX03C10_FETCH_LSB_BYTE_EXP(VAL)	((VAL) & 0xFF)	/* 8 Bits */

#define OX03C10_FETCH_LSB_GAIN(VAL)	(((VAL) << 4) & 0xf0)
#define OX03C10_FETCH_MSB_GAIN(VAL)	(((VAL) >> 4) & 0x1f)

#define OX03C10_REG_TEST_PATTERN	0x5080
...
static const struct regval ox03c10_non_combined_hdr_1920x1280_regs[] = {
	{ 0x0103, 0x01 },
	{ 0x0107, 0x01 },
	{ 0x4d5a, 0x1c },
	{ 0x4d09, 0xff },
...
ox03c10_find_best_fit(struct ox03c10 *ox03c10, struct v4l2_subdev_format *fmt)
{
	...
	for (i = 0; i < ox03c10->cfg_num; i++) {
		dist = ox03c10_get_reso_dist(&ox03c10->supported_modes[i],
					     framefmt);
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		dst_pixel_rate =
			...
			mode->bpp * 2 * lanes;
		__v4l2_ctrl_s_ctrl_int64(ox03c10->pixel_rate, dst_pixel_rate);
...
static int ox03c10_enable_test_pattern(struct ox03c10 *ox03c10, u32 pattern)
{
	...
	if (pattern)
		val = ((pattern - 1) << 2) | OX03C10_TEST_PATTERN_ENABLE;
	else
		val = OX03C10_TEST_PATTERN_DISABLE;
	ret = ox03c10_write_reg(ox03c10->client, OX03C10_REG_TEST_PATTERN,
				OX03C10_REG_VALUE_08BIT, val);
	ret |= ox03c10_write_reg(ox03c10->client,
				 OX03C10_REG_TEST_PATTERN + 0x40,
				 OX03C10_REG_VALUE_08BIT, val);
	return ret;
}
			     ...
			     struct preisp_hdrae_exp_s *ae)
{
	...
	if (!ox03c10->has_init_exp && !ox03c10->streaming) {
		...
		if (ox03c10->init_hdrae_exp.short_exp_reg >= 0x90) {
			dev_err(&ox03c10->client->dev,
				"short exposure must less than 0x90 before start stream!\n");
			return -EINVAL;
		}
		dev_dbg(&ox03c10->client->dev,
			"ox03c10 don't stream, record exp for hdr!\n");
		return ret;
	}
	...
	dev_dbg(&ox03c10->client->dev,
		...
		l_exp_time, l_a_gain, m_exp_time, m_a_gain, s_exp_time,
		s_a_gain);

	if (ox03c10->cur_mode->hdr_mode == HDR_X2) {
		//2 stagger
		l_a_gain = m_a_gain;
	...
	if (ox03c10->cur_mode->hdr_mode == HDR_X3 && s_a_gain > 248) {
	...
	ret |= ox03c10_write_reg(ox03c10->client, OX03C10_REG_DGAIN_MID_H,
				 OX03C10_REG_VALUE_24BIT,
				 (m_d_gain << 6) & 0xfffc0);
	ret |= ox03c10_write_reg(ox03c10->client, OX03C10_REG_EXP_MID_H,
				 OX03C10_REG_VALUE_16BIT, m_exp_time);
	if (ox03c10->cur_mode->hdr_mode == HDR_X3) {
		//3 stagger
		ret |= ox03c10_write_reg(ox03c10->client,
...
static long ox03c10_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		...
		for (i = 0; i < ox03c10->cfg_num; i++) {
			if (w == ox03c10->supported_modes[i].width &&
			    h == ox03c10->supported_modes[i].height &&
			    ox03c10->supported_modes[i].hdr_mode ==
				    hdr_cfg->hdr_mode) {
				ox03c10->cur_mode =
					&ox03c10->supported_modes[i];
				break;
			}
		}
		if (i == ox03c10->cfg_num) {
			dev_err(&ox03c10->client->dev,
			...
			dst_pixel_rate =
				(u32)link_freq_menu_items[mode->link_freq_idx] /
				mode->bpp * 2 * lanes;
			__v4l2_ctrl_s_ctrl_int64(ox03c10->pixel_rate,
						 dst_pixel_rate);
			__v4l2_ctrl_s_ctrl(ox03c10->link_freq, dst_link_freq);
			dev_info(&ox03c10->client->dev, "sensor mode: %d\n",
				 ox03c10->cur_mode->hdr_mode);
		}
		break;
...
static int __ox03c10_start_stream(struct ox03c10 *ox03c10)
{
	...
	if (!ox03c10->is_thunderboot) {
		ret = ox03c10_write_array(ox03c10->client,
					  ox03c10->cur_mode->reg_list);
		if (ret)
			return ret;
	}
	...
	if (ox03c10->has_init_exp && ox03c10->cur_mode->hdr_mode != NO_HDR) {
		ret = ox03c10_ioctl(&ox03c10->subdev, PREISP_CMD_SET_HDRAE_EXP,
...
static int __ox03c10_stop_stream(struct ox03c10 *ox03c10)
{
	...
	if (ox03c10->is_thunderboot)
		ox03c10->is_first_streamoff = true;
	return ox03c10_write_reg(ox03c10->client, OX03C10_REG_CTRL_MODE,
				 OX03C10_REG_VALUE_08BIT,
				 OX03C10_MODE_SW_STANDBY);
}
...
static int ox03c10_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		if (!ox03c10->is_thunderboot) {
			ret |= ox03c10_write_reg(ox03c10->client,
						 OX03C10_SOFTWARE_RESET_REG,
						 OX03C10_REG_VALUE_08BIT, 0x01);
			usleep_range(100, 200);
			ret |= ox03c10_write_array(
				ox03c10->client,
				ox03c10->cur_mode->global_reg_list);
			if (ret) {
				dev_err(&ox03c10->client->dev,
...
static int __ox03c10_power_on(struct ox03c10 *ox03c10)
{
	...
	if (!IS_ERR(ox03c10->reset_gpio))
		gpiod_direction_output(ox03c10->reset_gpio, 1);
...
static void __ox03c10_power_off(struct ox03c10 *ox03c10)
{
	...
	if (ox03c10->is_thunderboot) {
		if (ox03c10->is_first_streamoff) {
			ox03c10->is_thunderboot = false;
			ox03c10->is_first_streamoff = false;
		} else {
			return;
		}
	}
	...
	if (ox03c10->is_thunderboot_ng) {
		ox03c10->is_thunderboot_ng = false;
		regulator_bulk_disable(OX03C10_NUM_SUPPLIES, ox03c10->supplies);
	}
	usleep_range(30000, 31000);
}

static int __maybe_unused ox03c10_runtime_resume(struct device *dev)
{
...
static int __maybe_unused ox03c10_runtime_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct ox03c10 *ox03c10 = to_ox03c10(sd);
			    ...
			    struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	fie->reserved[0] = ox03c10->supported_modes[fie->index].hdr_mode;
	return 0;
}
...
static int ox03c10_initialize_controls(struct ox03c10 *ox03c10)
{
	...
	ox03c10->test_pattern = v4l2_ctrl_new_std_menu_items(
		handler, &ox03c10_ctrl_ops, V4L2_CID_TEST_PATTERN,
		ARRAY_SIZE(ox03c10_test_pattern_menu) - 1, 0, 0,
		ox03c10_test_pattern_menu);
			 ...
			 const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ox03c10->cfg_num; i++) {
		if (hdr_mode == supported_modes[i].hdr_mode) {
			ox03c10->cur_mode = &ox03c10->supported_modes[i];
			break;
		}
	}
	...
	ox03c10->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_ASIS);
	if (IS_ERR(ox03c10->reset_gpio))
